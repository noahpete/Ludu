...
namespace Ludu
{
    ...
    Ludu::VulkanTexture::VulkanTexture(const std::string &filepath)
    {
    }

    VulkanTexture::~VulkanTexture()
    {
        vkDestroyImage(VulkanDevice::Get().device(), m_Image, nullptr);
        vkFreeMemory(VulkanDevice::Get().device(), m_ImageMemory, nullptr);
    }

    void Ludu::VulkanTexture::CreateTextureImage(const std::string &filepath)
    {
        ...
        VulkanDevice::Get().copyBufferToImage(stagingBuffer.getBuffer(), m_Image, width, height, 1);
    ...
    void VulkanTexture::CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage &image, VkDeviceMemory &imageMemory)
    {
        ...
        if (vkCreateImage(VulkanDevice::Get().device(), &imageInfo, nullptr, &image) != VK_SUCCESS)
        {
            throw std::runtime_error("failed to create image!");
        }

        VkMemoryRequirements memRequirements;
        vkGetImageMemoryRequirements(VulkanDevice::Get().device(), image, &memRequirements);
        ...
        if (vkAllocateMemory(VulkanDevice::Get().device(), &allocInfo, nullptr, &imageMemory) != VK_SUCCESS)
        {
            throw std::runtime_error("failed to allocate image memory!");
        }

        vkBindImageMemory(VulkanDevice::Get().device(), image, imageMemory, 0);
    }
